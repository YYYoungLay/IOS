...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    //指定选择的cell，并在label中显示信息
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 0 {
            chosenLabel.text = "You have chosen teacher \(teachers[indexPath.row].fullName)"
            print("indexPath:\(indexPath),indexPath.row:\(indexPath.row)")
        } else {
            chosenLabel.text = "You have chosen student \(students[indexPath.row].fullName)"
            print("indexPath:\(indexPath),indexPath.row:\(indexPath.row)")
        }
    }
...
//        NotificationCenter.default.addObserver(self, selector: #selector(keyboardDidShow), name: .UIKeyboardDidShow, object: view.window)
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        //限制只能在自己的section藜麦移动，否则就会回到原来的文位置
...
//            if let rect = userInfo[UIKeyboardFrameBeginUserInfoKey] as? CGRect {
//                self.view.window?.frame.origin.y = -rect.height
//            }
//            print("keyboard 调用成功")
//        }
//
